...
export default function DashboardLayout({ children, role, userName }: DashboardLayoutProps) {
  ...
  const handleLogout = async () => {
    localStorage.removeItem("drcso_auto_login");
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="flex-1 md:ml-64">
        {/* 상단 헤더 */}
        <header className="sticky top-0 z-20 flex h-14 items-center justify-end border-b border-gray-200 bg-white px-4 sm:px-6">
        ...
        <div className="p-4 pt-14 sm:p-6 sm:pt-6 md:p-8">{children}</div>
      </main>
    </div>
  );
}
